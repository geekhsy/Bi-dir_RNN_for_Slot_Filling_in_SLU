...
import numpy as np
from tensorflow.contrib.rnn import DropoutWrapper,BasicRNNCell,LSTMCell, LSTMStateTuple
import sys
...
class Model_basic:
    def __init__(self, input_steps, embedding_size, hidden_size, vocab_size, slot_size,
        ...
        self.encoder_inputs = tf.placeholder(tf.int32, [input_steps, batch_size],
                                             name='encoder_inputs')
        # The actual length of each sentence, except padding
    ...
    def build(self):
        print('Building model with Basic RNN cell')
        ...
        # Encoder
        # Use a single Basic RNN cell
        encoder_f_cell_0 = BasicRNNCell(self.hidden_size)
        encoder_b_cell_0 = BasicRNNCell(self.hidden_size)
        ...
        def decode(helper, scope, reuse=None):
            with tf.variable_scope(scope, reuse=reuse):
                ...
                cell = tf.contrib.rnn.BasicRNNCell(num_units=self.hidden_size * 2)
...
class Model_LSTM:
    ...
    def build(self):
        print('Building model with Basic LSTM cell')
        self.embeddings = tf.Variable(tf.random_uniform([self.vocab_size, self.embedding_size],
                                                        -0.1, 0.1), dtype=tf.float32, name="embedding")
